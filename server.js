...
client.login(process.env.DISCORD_TOKEN);import { Client, GatewayIntentBits, REST, Routes, SlashCommandBuilder, PermissionFlagsBits, EmbedBuilder } from 'discord.js';
import express from 'express';
import cors from 'cors';
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'clear') {
        ...
        try {
            ...
            await interaction.reply({ content: 'Błąd usuwania', ephemeral: true });
        }
    }
});
...
app.post('/github-webhook', async (req, res) => {
    try {
        const data = req.body;
        if (!data.commits || data.commits.length === 0) return res.status(200).send('OK');
        const channel = await client.channels.fetch(ANNOUNCEMENT_CHANNEL_ID).catch(() => null);
        if (!channel) return res.status(404).send('Error');
